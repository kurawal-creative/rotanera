...
export const Canvas = forwardRef<CanvasRef, CanvasProps>(function Canvas({ width, height, canvasRatio, activeTool, color, lineWidth, fontSize, initialImage, onHistoryChange, floatingImage, onFloatingImageChange, onCommitFloatingImage, onCancelFloatingImage, isFullscreen = false }, ref) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const containerRef = useRef<HTMLDivElement>(null);

    // History hook
    const { history, historyIndex, saveToHistory, undo, redo, resetHistory } = useCanvasHistory({
        canvasRef,
        onHistoryChange,
    });

    // Drawing hook
    const { textInput, textValue, textInputRef, setTextValue, getScale, startDrawing, draw, stopDrawing, handleTextSubmit, cancelTextInput } = useCanvasDrawing({
        canvasRef,
        activeTool,
        color,
        lineWidth,
    ...
    // Floating image hook
    const { isDragging, getScreenCoords, handleMouseDown, handleResizeMouseDown } = useFloatingImage({
        floatingImage,
    ...
    const exportImage = useCallback((): string => {
        ...
        exportCtx.drawImage(canvas, 0, 0, canvas.width, canvas.height, 0, 0, ratioConfig.exportWidth, ratioConfig.exportHeight);
    ...
    const commitFloatingImage = useCallback(() => {
        ...
        ctx.drawImage(floatingImage.element, floatingImage.x, floatingImage.y, floatingImage.width, floatingImage.height);
        saveToHistory();
    ...
    return (
        <div ref={containerRef} className={`relative flex w-full justify-center bg-neutral-100 p-2 sm:p-4 dark:bg-neutral-900/50 ${isFullscreen ? "h-full items-center" : ""}`}>
            ...
            {floatingImage && screenCoords && <FloatingImageOverlay floatingImage={floatingImage} screenCoords={screenCoords} isDragging={isDragging} onMouseDown={handleMouseDown} onResizeMouseDown={handleResizeMouseDown} onCommit={commitFloatingImage} onCancel={onCancelFloatingImage} />}
            ...
            {textInput.visible && (
                <input
                    ...
                    className="min-w-[100px] border border-dashed border-purple-500 bg-transparent px-1 outline-none dark:border-purple-400"
