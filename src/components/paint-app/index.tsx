...
import { type PaintAppProps, type PaintAppRef, type ToolType, type FloatingImage, type CanvasRatio, DEFAULT_TOOLS, CANVAS_RATIOS } from "./types";

export const PaintApp = forwardRef<PaintAppRef, PaintAppProps>(function PaintApp({ onOutput, initialImage, width, height, tools = DEFAULT_TOOLS, onReset }, ref) {
        ...
        <div ref={containerRef} className={`mx-auto w-full ${isFullscreen ? "flex h-screen flex-col bg-neutral-100 dark:bg-neutral-900" : "max-w-4xl"}`}>
            <div className={`overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-lg dark:border-neutral-700 dark:bg-neutral-900 ${isFullscreen ? "flex flex-1 flex-col" : ""}`}>
                <div className="relative">
                    ...
                    <button onClick={toggleFullscreen} className="absolute top-1/2 right-2 -translate-y-1/2 rounded p-2 text-neutral-700 transition-colors hover:bg-neutral-100 dark:text-neutral-200 dark:hover:bg-neutral-700" title={isFullscreen ? "Keluar Fullscreen (Esc)" : "Fullscreen"} type="button">
                        {isFullscreen ? <Minimize2 className="h-5 w-5" /> : <Maximize2 className="h-5 w-5" />}
                    </button>
                </div>
                <div className={isFullscreen ? "flex flex-1 items-center justify-center overflow-auto" : ""}>
                    <Canvas
                        ref={canvasRef}
...
export type { PaintAppProps, PaintAppRef, ToolType, CanvasRatio } from "./types";
